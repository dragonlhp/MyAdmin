 ...
 * @since       1.0
 */
namespace app\modules\informationdb\controllers;
...
class InformationdbController extends QrckController
{
    ...
    public function actionIndex(){
        ...
        $model = BsSeniorCompany::find()->select('member_id as id ,name,reg_date,reg_capital,legal_rep,link_name,incubator_id')->where(['status'=>2]);
...
//            $model->andWhere(['like','name',$keyword]);
        ...
        foreach ($data as $key => $item) {
                $incubator = BsMemberIncubator::find()->where(['member_id'=>$item['id']])->one();
                $datas[$key] = $item;
                $datas[$key]['incubname'] = $incubator['name'] ;
        }
    ...
    public function actionCheckstatus()
    {
        ...
        if (empty($ids) || empty($check_status)) {
            ...
            foreach ($ids as $i) {
                ...
                if($check_status == 3){
                    BsSdApply::updateAll(['status'=>3,'check_status'=>3],['id'=>$i]);

//                    $model->setattribute("status", 3);
//                    $model->setattribute("check_status", 3);
                }elseif ($check_status == 2){

//                    $model->setattribute("check_status", 2);
                    $date = date('Y-m-d');
                    BsSdApply::updateAll(['check_status'=>2,'cooperation'=>$date],['id'=>$i]);
