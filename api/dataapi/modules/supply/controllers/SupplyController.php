 ...
 * @since       1.0
 */
namespace app\modules\supply\controllers;
...
class SupplyController extends QrckController
{
...
//                    1：载体  2：资金 3：技术 4：专利 5：产品
                    BsSdCarrierSup::updateAll(['status'=>2],['id'=>$val['id']]);
                }elseif ($val['pro_type']==2){
                    BsSdCapitalSup::updateAll(['status'=>2],['id'=>$val['id']]);
                }elseif ($val['pro_type']==3){
                    BsSdSkillSup::updateAll(['status'=>2],['id'=>$val['id']]);
                }elseif ($val['pro_type']==4){
                    BsSdPatentSup::updateAll(['status'=>2],['id'=>$val['id']]);
                }elseif ($val['pro_type']==5){
                    BsSdProductSup::updateAll(['status'=>2],['id'=>$val['id']]);
                }
            }
    ...
    public function actionStatus(){
        ...
        //1：载体  2：资金 3：技术 4：专利 5：产品
            ...
            if($model){
                $res = BsSdCarrierSup::updateAll(['status'=>$status],['id'=>$id]);
                if ($res) {
                    return $res;
                } elseif (!$model->hasErrors()) {
            ...
            if($model){
                $res = BsSdCapitalSup::updateAll(['status'=>$status],['id'=>$id]);
                if ($res) {
                    return $res;
                } elseif (!$model->hasErrors()) {
                    throw new ServerErrorHttpException('删除失败。');
            ...
            if($model){
                $res = BsSdSkillSup::updateAll(['status'=>$status],['id'=>$id]);
                if ($res) {
                    return $res;
                } elseif (!$model->hasErrors()) {
                    throw new ServerErrorHttpException('删除失败。');
            ...
            if($model){
                $res = BsSdPatentSup::updateAll(['status'=>$status],['id'=>$id]);
                if ($res) {
                    return $res;
                } elseif (!$model->hasErrors()) {
                    throw new ServerErrorHttpException('删除失败。');
            ...
            if($model){
                $res = BsSdProductSup::updateAll(['status'=>$status],['id'=>$id]);
                if ($res) {
                    return $res;
                } elseif (!$model->hasErrors()) {
                    throw new ServerErrorHttpException('删除失败。');
